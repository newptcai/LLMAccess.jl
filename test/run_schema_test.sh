...
# Run the command gpt models for openrouter
# julia --project="$PROJECT_ROOT" "$ASK_JL_SCRIPT" --llm openrouter --model "openai/gpt-4.1-nano" --schema "$TEST_SCHEMA" "$PROMPT" | jq

# Run the command for gpt-oss models via openrouter
# julia --project="$PROJECT_ROOT" "$ASK_JL_SCRIPT" --llm openrouter --model "openai/gpt-oss-20b:free" --schema "$TEST_SCHEMA" "$PROMPT" | jq
